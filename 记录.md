...
- 1️⃣ 创建一个描述符数组
- 2️⃣ 为每个中断号填充对应的描述符，包括处理函数的地址、代码段选择子和权限属性
- 3️⃣ 用lidt指令将表的地址和大小加载到CPU。这样当中断发生时，CPU就能自动查找并跳转到你指定的处理函数。
...
- 1️⃣ 对 26 - 01 - 09 的工作进行收尾
- 2️⃣ 增加系统调用 并在os_init中补充其起始地址的设置
- 3️⃣ limit_l 指定系统调用处理函数syscall_handler 在C文件和汇编文件提前声明
- 4️⃣ 在汇编函数增加 syscall_handler标签
- 5️⃣ 增加一个通用的函数 do_syscall, 这个函数是在syscall_handler函数里面使用的
- 6️⃣ 增加一个函数 sys_show, 这个函数主要功能就是显示字符串 涉及到一些在C语言内调用汇编的操作

> **烂尾两天** ➡️ 11号早上完成了系统调用功能 但没记录，12号考试后摆烂一天，13号仅1个多小时
...
- 添加boot工程➡️寄存器初始化 + 读取磁盘扇区
- 添加loader子工程：
  - 1️⃣ boot工程首先在汇编执行了x86保护模式内容
  - 2️⃣ 后通过C文件跳转到 boot_entry 函数当中
  - 3️⃣ 定义宏，指向loader工程的执行程序在内存中的首地址
  - 4️⃣ 通过宏（地址），定义函数指针，并通过该指针在 boot_entry 函数中实现到 loader 程序的跳转
...
#### 创建内核文件
> 找Bug，无法从call kernel_init 跳转到c代码当中
> 似乎解决了 但这是过了几天后自动解决的？
